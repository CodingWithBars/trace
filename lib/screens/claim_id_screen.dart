...
class _ClaimIdScreenState extends State<ClaimIdScreen> {
  ...
  Future<void> _checkIdExists(String id) async {
    if (id.trim().isEmpty) { setState(() => _idExistsError = null); return; }
    setState(() { _checkingId = true; _idExistsError = null; });
    final student = await StudentService.getStudentByStudentId(id.trim());
    if (mounted) {
      setState(() {
        _checkingId = false;
        _idExistsError = student == null ? 'Student ID not found in the system. Make sure the ID is correct.' : null;
      });
    }
  }

  Future<void> _pickProof() async {
    final file = await ImagePicker().pickImage(source: ImageSource.gallery, imageQuality: 75, maxWidth: 1200);
    if (file == null) return;
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            ...
            decoration: BoxDecoration(shape: BoxShape.circle, color: TraceColors.success.withValues(alpha: 0.1)),
            child: const Icon(Icons.check_circle_rounded, size: 60, color: TraceColors.success),
          ),
          const SizedBox(height: 24),
          Text('Claim Submitted!', style: GoogleFonts.inter(fontSize: 26, fontWeight: FontWeight.w800, color: TraceColors.navyBlue)),
          ...
          Text(
            ...
            style: GoogleFonts.inter(fontSize: 15, color: TraceColors.medGrey, height: 1.6),
          ),
          const SizedBox(height: 32),
          GoldButton(label: 'Go to Home', icon: Icons.home_outlined, onPressed: () => context.go('/')),
        ]),
      ),
    );
  }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: TraceColors.royalBlue.withValues(alpha: 0.2)),
            ),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Icon(Icons.info_outline, color: TraceColors.royalBlue, size: 20),
          ...
          _textField(
            ...
            suffix: _checkingId ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : null,
          ...
          TextFormField(
            ...
            maxLines: 5,
            decoration: _inputDecoration('Explain why you are the rightful owner of this Student ID...'),
          ...
          Text('Upload a photo of your school ID or any document proving your identity.',
            style: GoogleFonts.inter(fontSize: 12, color: TraceColors.medGrey)),
          const SizedBox(height: 8),
          GestureDetector(
            onTap: _pickProof,
            child: Container(
              width: double.infinity, height: _proofBytes != null ? 180 : 100,
              decoration: BoxDecoration(
              ...
              child: _proofBytes != null
                  ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(_proofBytes!, fit: BoxFit.cover))
                  : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      const Icon(Icons.upload_file_rounded, size: 36, color: TraceColors.medGrey),
                      const SizedBox(height: 8),
                      Text('Tap to upload', style: GoogleFonts.inter(color: TraceColors.medGrey, fontSize: 13)),
                    ]),
            ),
          ),
          ...
          SizedBox(width: double.infinity,
            child: ElevatedButton(
              ...
              child: _isSubmitting
                  ...
                  : Text('Submit Claim Petition', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white)),
            )),
          const SizedBox(height: 24),
        ]),
      ),
  ...
  Widget _sectionLabel(String label) => Padding(
    ...
    child: Text(label, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700, color: TraceColors.navyBlue)),
  );
