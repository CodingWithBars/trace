...
class _StudentIdScreenState extends State<StudentIdScreen> {
    ...
    if (_student == null) {
      return Scaffold(
        ...
        appBar: TraceAppBar(title: 'My ID', actions: [
          IconButton(onPressed: () => context.go('/'), icon: const Icon(Icons.home_outlined, color: TraceColors.gold)),
        ]),
        body: Center(child: Text('Student not found.', style: GoogleFonts.inter(fontSize: 18, color: TraceColors.error))),
      );
    }
  ...
  Widget _buildAvatarWidget(String url) {
    if (url.isEmpty) return const Icon(Icons.person, size: 54, color: TraceColors.lightGrey);
    if (url.startsWith('data:image')) {
      try { return Image.memory(base64Decode(url.split(',').last), fit: BoxFit.cover); } catch (_) {}
    }
    return Image.network(url, fit: BoxFit.cover,
        errorBuilder: (_, _, _) => const Icon(Icons.person, size: 54, color: TraceColors.lightGrey));
  }
}
...
class _EditProfileSheetState extends State<_EditProfileSheet> {
  ...
  Future<void> _pickImage() async {
    final file = await ImagePicker().pickImage(source: ImageSource.gallery, imageQuality: 80, maxWidth: 800);
    if (file == null) return;
  ...
  Future<void> _onIdChanged(String val) async {
    if (val.trim() == widget.student.studentId) { setState(() => _idError = null); return; }
    setState(() { _isCheckingId = true; _idError = null; });
    final taken = await StudentService.isStudentIdTaken(val.trim(), excludeDocId: widget.student.id);
    if (mounted) setState(() { _isCheckingId = false; _idError = taken ? 'ID already used' : null; });
  }

  Future<void> _save() async {
    final name = _nameCtrl.text.trim();
    final id = _idCtrl.text.trim();
    if (name.isEmpty || id.isEmpty) { setState(() => _generalError = 'Name and Student ID cannot be empty.'); return; }
    if (_idError != null) return;
    setState(() { _isSaving = true; _generalError = null; });
    try {
      if (id != widget.student.studentId) {
        final taken = await StudentService.isStudentIdTaken(id, excludeDocId: widget.student.id);
        if (taken) { setState(() { _idError = 'ID already used'; _isSaving = false; }); return; }
      }
      String? newAvatarUrl;
      if (_pickedBytes != null) {
        try { newAvatarUrl = await StudentService.uploadAvatar(_pickedBytes!, id) ?? _pickedBase64; }
        catch (_) { newAvatarUrl = _pickedBase64; }
      }
      await StudentService.updateStudentProfile(docId: widget.student.id, name: name, studentId: id, avatarUrl: newAvatarUrl);
      final updated = Student(
        id: widget.student.id, studentId: id, name: name,
        course: widget.student.course, yearLevel: widget.student.yearLevel,
        qrHash: widget.student.qrHash, email: widget.student.email,
      ...
      setState(() { _generalError = 'Failed to save: $e'; _isSaving = false; });
    }
  }
  ...
  Widget _avatarPreview() {
    ...
    return Image.network(url, fit: BoxFit.cover,
        errorBuilder: (_, _, _) => const Icon(Icons.person, size: 40, color: TraceColors.lightGrey));
  }
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12),
                borderSide: BorderSide(color: _idError != null ? TraceColors.error : TraceColors.navyBlue, width: 2)),
            errorText: _idError,
            contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            suffixIcon: _isCheckingId
                ? const Padding(padding: EdgeInsets.all(12), child: SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)))
                : (_idError == null && _idCtrl.text.isNotEmpty ? const Icon(Icons.check_circle, color: TraceColors.success) : null),
          ),
          style: GoogleFonts.inter(fontSize: 15, color: TraceColors.navyBlue),
        ),
